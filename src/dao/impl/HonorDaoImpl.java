...
public class HonorDaoImpl implements IHonorDao{
	...
	public int insertHonor(Honor honor) throws SQLException {		//插入新的荣誉信息
		...
		List params = Arrays.asList(honor.getHsn(),honor.getHname(),honor.getHwinner(),Hdate,honor.getHcompany(),honor.getHgrad(),
				honor.getHreward(),honor.getHremarks(),Tsn);
		System.out.println(dbUtil.getUpdateResult(sql, params));
		return dbUtil.getUpdateResult(sql, params);
	}
		...
		//college：所属学院，sdept：所属系，endtime：截止时间，Tname：搜索的教师名
		@Override
		public ResultSet selectCollegeHonor(String college,String sdept,String endtime,String Tname,int currentPage,int pageSize) throws SQLException {		//根据查询条件的改变显示不同的查询结果
			...
			//什么都没有设置
			String sql1 = "select * from (select COUNT(*)OVER() AS totalRecord,Hsn,Hname,Hwinner,Hdate,Hcompany,Hgrad,Hreward,Hremarks "
					+ ",ROW_NUMBER() over (order by Hsn) as r from Honor h left join Teacher t on h.Tsn = t.Tsn ) as pp where pp.r "
					+ "between ? and ?";
			//设置了要查询的学院
			String sql2 = "select * from (select COUNT(*)OVER() AS totalRecord,Hsn,Hname,Hwinner,Hdate,Hcompany,Hgrad,Hreward,Hremarks,Cname ," + 
			...
			//设置了要查询的学院和专业
			String sql3 = "select * from (select COUNT(*)OVER() AS totalRecord,Hsn,Hname,Hwinner,Hdate,Hcompany,Hgrad,Hreward,Hremarks,Dname ," 
			...
			//设置了要查询的起止时间
			String sql4 = "select * from (select COUNT(*)OVER() AS totalRecord,Hsn,Hname,Hwinner,Hdate,Hcompany,Hgrad,Hreward,Hremarks  ," + 
			...
			//设置了要查询的学院和起止时间
			String sql5 = "select * from (select COUNT(*)OVER() AS totalRecord,Hsn,Hname,Hwinner,Hdate,Hcompany,Hgrad,Hreward,Hremarks,Cname ,"
			...
			//设置了要查询的学院和专业和起止时间
			String sql6 = "select * from (select COUNT(*)OVER() AS totalRecord,Hsn,Hname,Hwinner,Hdate,Hcompany,Hgrad,Hreward,Hremarks,Dname ,"
					+	"ROW_NUMBER() over (order by Hsn) as r from Honor h left join Teacher t on h.Tsn = t.Tsn join College c "
					+ "on t.Csn = c.Csn join Sdept s on t.Dsn = s.Dsn where Cname = ? and Dname = ? and Hdate <= ? ) as pp where pp.r between ? and ?";	
			//没有设置要查询的教师名
			String sql7 = "select * from (select COUNT(*)OVER() AS totalRecord,Hsn,Hname,Hwinner,Hdate,Hcompany,Hgrad,Hreward,Hremarks  ," + 
					"ROW_NUMBER() over (order by Hsn) as r from Honor h left join Teacher t on h.Tsn = t.Tsn where Tname = ?) "
					+ "as pp where pp.r between ? and ?";
			//设置了要查询的学院和教师名
			String sql8 = "select * from (select COUNT(*)OVER() AS totalRecord,Hsn,Hname,Hwinner,Hdate,Hcompany,Hgrad,Hreward,Hremarks,Cname  ," + 
					"ROW_NUMBER() over (order by Hsn) as r from Honor h left join Teacher t on h.Tsn = t.Tsn join College c "
					+ "on t.Csn = c.Csn where c.Cname = ? and Tname = ?) as pp where pp.r between ? and ?";
			//没有设置要查询的学院和专业和教师名
			String sql9 = "select * from (select COUNT(*)OVER() AS totalRecord,Hsn,Hname,Hwinner,Hdate,Hcompany,Hgrad,Hreward,Hremarks,Dname" + 
					"ROW_NUMBER() over (order by Hsn) as r from Honor h left join Teacher t on h.Tsn = t.Tsn join College c "
			...
			//设置了要查询的学院和起止时间和教师名
			String sql11 = "select * from (select COUNT(*)OVER() AS totalRecord,Hsn,Hname,Hwinner,Hdate,Hcompany,Hgrad,Hreward,Hremarks,Cname ," + 
			...
			String sql12 = "select * from (select COUNT(*)OVER() AS totalRecord,Hsn,Hname,Hwinner,Hdate,Hcompany,Hgrad,Hreward,Hremarks,Cname,Dname" + 
					"ROW_NUMBER() over (order by Hsn) as r from Honor h left join Teacher t on h.Tsn = t.Tsn join College c on t.Csn = c.Csn"
			...
			try {
				while(rs.next()) {
					datalist.add(new Honor(rs.getString("Hsn"),rs.getString("Hname"), rs.getString("Hwinner"),rs.getDate("Hdate"),
							rs.getString("Hcompany"),rs.getString("Hgrad"),rs.getString("Hremarks"),rs.getString("Hremarks"),rs.getInt("totalRecord")));
				}
				return datalist;
			} catch (SQLException e) {
				e.printStackTrace();
