	...
	public int[] getPage(int currentPage,int totalPage) {
		if(currentPage == 1 || currentPage == 2) {																//当前页为前两页
			...
			return pageArr;
 			...
 			int [] pageArr = {currentPage - 1,currentPage ,currentPage + 1};	//当前页为倒数第二页
 			return pageArr;
 		}else if(totalPage == currentPage) {
 			int [] pageArr = {currentPage - 2,currentPage - 1,currentPage};		//当前页为倒数第一页
			return pageArr;
 		}else {																									//一般情况
 			int [] pageArr = {currentPage - 1,currentPage,currentPage + 1};
			return pageArr;
 		}
	}
